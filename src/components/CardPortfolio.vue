    ...
    <a :href="`/photos/${name.toLowerCase()}`">
        <div class="relative group w-full">
            ...
            <div class="absolute inset-0 flex items-end justify-center pointer-events-none">
                <span
                    class="mb-6 text-white lg:text-3xl sm:text-2xl text-xl font-mini_titre px-6 py-2 rounded-lg opacity-0 translate-y-8 transition-all duration-500 ease-in-out group-hover:opacity-100 group-hover:translate-y-0">
                    {{ name }}
                </span>
            </div>
        </div>
